...
class LocalUpdater(object):
    ...
    def _get_omegas(self, t, jac_mats, fx_t, state_dict):
        ...
        def jac_col(jac_mats, col_idx):
            # Selects columns of the jac matrices
            jac = OrderedDict()
...
def get_omegas(t, lr, jac_mats, 
    ...
    # Computes Residuals based on loss type
    if loss_type == "ce":
    ...
    # Same as eq. 9b
    # Grabs a column of the jacobian matrix, multiplies by respective residuals
    acc_omega = WeightMod(jac_col(jac_mats, 0, "cuda"))
    acc_omega.mat_mul(residuals[:,0].cuda(), "ij...,i->j...")
    # Finishes the rest of the summation
