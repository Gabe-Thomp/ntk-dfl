...
def train(config, logger, record, loaded_record):
    ...
    def train_client(client_id, neighbors, model_dict, record, config, dataset, verbose=False):
        ...
        # Affirm that the model is the client's model
        ...
        model = model_dict[client_id]
        # Load chosen weight into client model
        model.load_state_dict(params)

        # Now average the weights of the neighbors
        neighbor_averaged_weight = average_neighbor_weights(client_id, neighbors, model_dict)
        # Load aggregated weight into current client
        model.load_state_dict(neighbor_averaged_weight)
        
        # Assess new accuracy and loss
        output = model(test_images)
        new_test_acc = accuracy_with_output(output, test_labels)

        output = model(global_xs)    
        new_loss = loss_with_output(output, global_ys, config.loss)
        
        # Select tau with lowest loss
        current_tau = taus[idx]
        # current_acc = acc[idx]
        # Record new test acc
        current_acc = new_test_acc
    ...
    def complete_train(user_ids, comm_rounds, model_dict, record, config, dataset, verbose=False):
        ...
        if record["epoch"] == 0:
            if verbose: logger.info("Logging initial loss, acc, and tau")
            client_losses = []
            client_accs = []
            client_taus = []
